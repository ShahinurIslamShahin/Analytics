...
-- MSISDN_USSD_BI  (View) 
--
CREATE OR REPLACE FORCE VIEW DWH_USER.MSISDN_USSD_BI
(MSISDN, TIMESTAMP, LATITUDE, LONGITUDE, UPAZILA, 
 DISTRICT)
BEQUEATH DEFINER
AS 
SELECT AA.MSISDN ,AA.TIMESTAMP,AA.LATITUDE, AA.LONGITUDE, AA.UPAZILA, AA.DISTRICT 
FROM
(SELECT  XX.MSISDN, XX.TIMESTAMP ,XX.LATITUDE, XX.LONGITUDE, XX.UPAZILA,XX.DISTRICT,  RANK() OVER (PARTITION BY XX.MSISDN ORDER BY XX.TIMESTAMP DESC) AS LAST_KEY
FROM
(SELECT Z.V372_CALLINGPARTYNUMBER AS MSISDN, TO_CHAR(TO_DATE(Z.CALL_DATE,'YYYY-MM-DD HH24:MI:SS'),'YYYY-MM-DD HH24:MI:SS') AS TIMESTAMP ,Z.LATITUDE, Z.LONGITUDE, Z.UPAZILA,Z.DISTRICT
...
(SELECT K.V372_CALLINGPARTYNUMBER,K.DATE_VALUE||K.V387_CHARGINGTIME_HOUR AS CALL_DATE, M.LATITUDE, M.LONGITUDE, M.UPAZILA,M.DISTRICT
    FROM ZONE_DIM@DWH05TODWH01 M,
        ...
        WHERE V387_CHARGINGTIME_KEY IN (SELECT A.DATE_KEY FROM DATE_DIM A WHERE A.DATE_VALUE BETWEEN TO_DATE (SYSDATE-4,'DD/MM/RRRR') AND TO_DATE (SYSDATE-1,'DD/MM/RRRR'))
