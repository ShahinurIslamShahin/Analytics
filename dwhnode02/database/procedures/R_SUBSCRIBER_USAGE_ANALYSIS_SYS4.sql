--
-- R_SUBSCRIBER_USAGE_ANALYSIS_SYS4  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_SUBSCRIBER_USAGE_ANALYSIS_sys4 IS
    VDATE_KEY       NUMBER;
BEGIN
...
FROM L3_VOICE P
WHERE V378_SERVICEFLOW=1 AND V387_CHARGINGTIME_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-4,'DD/MM/RRRR'))                                                     
...
(SELECT  /*+PARALLEL(P,15)*/ R375_CHARGINGPARTYNUMBER,R373_MAINOFFERINGID,R377_CYCLEBEGINTIME_KEY,NULL AS VOICE_PAYG_REVENUE,SUM(R41_DEBIT_AMOUNT) VOICE_RECURRING_REVENUE, NULL AS MO_DURATION_MINS ,
         ...
         NULL AS SMS_PAYG_REVENUE,NULL AS SMS_BUNDLE_REVENUE,NULL AS MO_SMS_COUNT,NULL AS MT_SMS_COUNT,NULL AS COMBO_BUNDLE_REVENUE,NULL AS RECHARGE_AMOUNT
FROM L3_RECURRING P
WHERE R377_CYCLEBEGINTIME_KEY =  (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-4,'DD/MM/RRRR')) 
...
WHERE (PROCESSED_DATE BETWEEN  TO_DATE(SYSDATE-4,'DD/MONTH/RRRR') AND TO_DATE(SYSDATE-1,'DD/MONTH/RRRR'))
AND M07_ANSWERTIMESTAMP_KEY =(SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-4,'DD/MM/RRRR'))
...
(SELECT  /*+PARALLEL(P,8)*/ G372_CALLINGPARTYNUMBER,G401_MAINOFFERINGID,G383_CHARGINGTIME_KEY,NULL AS VOICE_PAYG_REVENUE,NULL AS VOICE_RECURRING_REVENUE, NULL AS MO_DURATION_MINS ,
         ...
         NULL AS SMS_PAYG_REVENUE,NULL AS SMS_BUNDLE_REVENUE,NULL AS MO_SMS_COUNT,NULL AS MT_SMS_COUNT,NULL AS COMBO_BUNDLE_REVENUE,NULL AS RECHARGE_AMOUNT
FROM L3_DATA P
WHERE G383_CHARGINGTIME_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-4,'DD/MM/RRRR'))                                                    
...
FROM L3_SMS P
WHERE  S387_CHARGINGTIME_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-4,'DD/MM/RRRR'))  
...
FROM L3_RECHARGE P
WHERE RE30_ENTRY_DATE_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-4,'DD/MM/RRRR'))                                                   
