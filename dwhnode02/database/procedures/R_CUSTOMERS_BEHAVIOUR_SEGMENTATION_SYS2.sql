--
-- R_CUSTOMERS_BEHAVIOUR_SEGMENTATION_SYS2  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_CUSTOMERS_BEHAVIOUR_SEGMENTATION_SYS2 IS
    VDATE_KEY       NUMBER;
BEGIN
...
WHERE  (R377_CYCLEBEGINTIME_KEY=  (SELECT DATE_KEY FROM DWH_USER.DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-2,'DD/MM/RRRR')))
...
WHERE (RE30_ENTRY_DATE_KEY = (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TRUNC(TO_DATE(SYSDATE-2,'DD/MM/RRRR'))))
...
WHERE S387_CHARGINGTIME_KEY  =(SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TRUNC(TO_DATE(SYSDATE-2,'DD/MM/RRRR')))
...
WHERE (PROCESSED_DATE BETWEEN TO_DATE(SYSDATE-2,'DD/MONTH/RRRR') AND TO_DATE(SYSDATE,'DD/MONTH/RRRR'))
AND TO_DATE(SUBSTR(P.S387_CHARGINGTIME,1,8),'RRRRMMDD')=Q.DATE_VALUE
AND (Q.DATE_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-2,'DD/MM/RRRR')))
...
WHERE G383_CHARGINGTIME_KEY  = (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TRUNC(TO_DATE(SYSDATE-2,'DD/MM/RRRR')))
