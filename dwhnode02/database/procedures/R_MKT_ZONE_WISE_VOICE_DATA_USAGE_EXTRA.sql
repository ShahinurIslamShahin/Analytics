--
-- R_MKT_ZONE_WISE_VOICE_DATA_USAGE_EXTRA  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_MKT_ZONE_WISE_VOICE_DATA_USAGE_extra IS
...
G384_TOTALFLUX
FROM L3_DATA P
WHERE  (G383_CHARGINGTIME_KEY BETWEEN (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('15/11/2020','DD/MM/RRRR'))
...
FROM  L3_VOICE  P
WHERE (V387_CHARGINGTIME_KEY BETWEEN (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('15/11/2020','DD/MM/RRRR'))
AND (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('18/11/2020','DD/MM/RRRR')))
