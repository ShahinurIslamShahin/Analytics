--
-- R_SUBSCRIBER_CHURN_RATE_EXTRA  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_SUBSCRIBER_CHURN_RATE_EXTRA IS
    VDATE_KEY       NUMBER;
BEGIN
...
--DELETE SUBSCRIBER_CHURN_RATE WHERE MONTH_KEY=(SELECT  MONTH_KEY-1 FROM DATE_DIM P WHERE  DATE_VALUE= TRUNC(TO_DATE('15/02/2021','DD/MM/RRRR')));
--COMMIT;
...
(SELECT /*+PARALLEL(P,15)*/ '202005' MONTH_KEY, R375_CHARGINGPARTYNUMBER,PRODUCT_ID,PRODUCT_NAME,SUM (R41_DEBIT_AMOUNT) MRR
FROM L3_RECURRING P,PRODUCT_DIM Q,DATE_DIM R
WHERE (R377_CYCLEBEGINTIME_KEY IN (SELECT DATE_KEY FROM DATE_DIM WHERE MONTH_KEY = '202004'))
...
FROM L3_RECURRING P,PRODUCT_DIM Q
WHERE (R377_CYCLEBEGINTIME_KEY IN (SELECT DATE_KEY FROM DATE_DIM WHERE MONTH_KEY ='202005'))
AND P.R373_MAINOFFERINGID=Q.PRODUCT_ID
