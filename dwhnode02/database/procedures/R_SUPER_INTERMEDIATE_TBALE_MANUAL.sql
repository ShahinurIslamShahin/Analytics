--
-- R_SUPER_INTERMEDIATE_TBALE_MANUAL  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_SUPER_INTERMEDIATE_TBALE_MANUAL IS
    VDATE_KEY       NUMBER;
BEGIN
...
FROM L3_DATA P
WHERE (G383_CHARGINGTIME_KEY BETWEEN (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('01/11/2020','DD/MM/RRRR'))
AND (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('30/11/2020','DD/MM/RRRR')))
...
FROM  L3_VOICE  P
WHERE (V387_CHARGINGTIME_KEY BETWEEN (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('01/11/2020','DD/MM/RRRR'))
AND (SELECT DATE_KEY FROM DATE_DIM WHERE DATE_VALUE = TO_DATE('30/11/2020','DD/MM/RRRR')))
...
WHERE (PROCESSED_DATE BETWEEN  TO_DATE('01/NOV/2020','DD/MONTH/RRRR') AND TO_DATE('02/DEC/2020','DD/MONTH/RRRR'))
--AND SUBSTR(M07_ANSWERTIMESTAMP,1,6) IN ('202006','202007','202008','202009','202010','202011','202012')
AND   M07_ANSWERTIMESTAMP_KEY BETWEEN ((SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE('01/11/2020','DD/MM/RRRR')))
                                                       AND  ((SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE('30/11/2020','DD/MM/RRRR')))
