--
-- R_SUBSCRIBER_USAGE_ANALYSIS_SYS2  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_SUBSCRIBER_USAGE_ANALYSIS_sys2 IS
    VDATE_KEY       NUMBER;
BEGIN
...
FROM L3_VOICE P
WHERE V378_SERVICEFLOW=1 AND V387_CHARGINGTIME_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-2,'DD/MM/RRRR'))                                                     
...
(SELECT  /*+PARALLEL(P,15)*/ R375_CHARGINGPARTYNUMBER,R373_MAINOFFERINGID,R377_CYCLEBEGINTIME_KEY,NULL AS VOICE_PAYG_REVENUE,SUM(R41_DEBIT_AMOUNT) VOICE_RECURRING_REVENUE, NULL AS MO_DURATION_MINS ,
         ...
         NULL AS SMS_PAYG_REVENUE,NULL AS SMS_BUNDLE_REVENUE,NULL AS MO_SMS_COUNT,NULL AS MT_SMS_COUNT,NULL AS COMBO_BUNDLE_REVENUE,NULL AS RECHARGE_AMOUNT
FROM L3_RECURRING P
WHERE R377_CYCLEBEGINTIME_KEY =  (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-2,'DD/MM/RRRR')) 
...
FROM L3_SMS P
WHERE  S387_CHARGINGTIME_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-2,'DD/MM/RRRR'))  
...
FROM L3_RECHARGE P
WHERE RE30_ENTRY_DATE_KEY = (SELECT DATE_KEY FROM DATE_DIM  WHERE DATE_VALUE = TO_DATE(SYSDATE-2,'DD/MM/RRRR'))                                                   
