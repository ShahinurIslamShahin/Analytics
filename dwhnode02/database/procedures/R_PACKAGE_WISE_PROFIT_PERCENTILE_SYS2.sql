--
-- R_PACKAGE_WISE_PROFIT_PERCENTILE_SYS2  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_PACKAGE_WISE_PROFIT_PERCENTILE_SYS2 IS
    VDATE_KEY       NUMBER;
BEGIN
...
(SELECT D.DATE_KEY,
  ...
  FROM (SELECT UNIQUE A.DATE_KEY, A.PRODUCT_ID
          ...
          where a.date_key=(select date_key from date_dim where date_value=to_date(sysdate-2,'dd/mm/rrrr'))
        UNION
        SELECT UNIQUE B.DATE_KEY, B.PRODUCT_ID
          FROM PACKAGE_WISE_SMS_PROFIT B
          where b.date_key=(select date_key from date_dim where date_value=to_date(sysdate-2,'dd/mm/rrrr'))
        UNION
        SELECT UNIQUE C.DATE_KEY, C.PRODUCT_ID
          FROM PACKAGE_WISE_DATA_PROFIT C
          where c.date_key=(select date_key from date_dim where date_value=to_date(sysdate-2,'dd/mm/rrrr'))) D
       LEFT OUTER JOIN
       (  SELECT V.DATE_KEY,
                 V.PRODUCT_ID,
            ...
            where v.date_key=(select date_key from date_dim where date_value=to_date(sysdate-2,'dd/mm/rrrr'))
       ...
       (  SELECT S.DATE_KEY,
            ...
            where s.date_key=(select date_key from date_dim where date_value=to_date(sysdate-2,'dd/mm/rrrr'))
       ...
       (  SELECT G.DATE_KEY,
            ...
            where g.date_key=(select date_key from date_dim where date_value=to_date(sysdate-2,'dd/mm/rrrr'))
