--
-- R_PACKAGE_WISE_VOICE_PROFIT_SYS2  (Procedure) 
--
CREATE OR REPLACE PROCEDURE DWH_USER.R_PACKAGE_WISE_VOICE_PROFIT_SYS2 IS
    VDATE_KEY       NUMBER;
BEGIN
...
SELECT a.DATE_KEY,
  ...
  FROM (SELECT DATE_KEY,
          ...
          FROM (  SELECT /*+PARALLEL(P,15)*/
                   ...
                   WHERE     (V387_CHARGINGTIME_KEY = (SELECT DATE_KEY
                                                               FROM DATE_DIM
                                                              WHERE DATE_VALUE =
                                                                       TRUNC (
                                                                          TO_DATE (
